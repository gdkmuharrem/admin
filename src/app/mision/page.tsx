...
export default function MisionPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (mision) {
        ...
        setMision(updated);
        setForm(updated);
        showNotification('Mision başarıyla güncellendi.', 'success');
      } else {
        const dto: CreateMisionDto = {
          title_tr: form.title_tr!,
          title_en: form.title_en!,
          contents: form.contents as ContentItem[],
        };
        const created = await misionService.createMision(dto);
        setMision(created);
        setForm(created);
        showNotification('Mision başarıyla oluşturuldu.', 'success');
      }
      setError('');
    } catch (error: unknown) {
  ...
  const handleDelete = async () => {
    if (!mision) return;
    if (!confirm('Mision yazısını ve ona bağlı tüm resimleri silmek istediğinize emin misiniz?')) return;
    ...
    try {
      ...
      setImages([]);
      setError('');
      showNotification('Mision ve tüm resimler başarıyla silindi.', 'success');
    } catch (error: unknown) {
      const msg = getErrorMessage(error);
      setError(msg);
